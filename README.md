...
* **终端入口**: 可直接从左侧滑出菜单进入终端页面。
* **交互式 Shell**: 内置可持续运行的 PTY Shell，会根据终端视口大小自动调整行列。
* **终端工具栏**: 提供 `Ctrl+C`、`Ctrl+D`、翻页、粘贴、清屏、重置等快捷操作。
* **字体缩放**: 支持在终端页内调整字号，并自动持久化到本地设置。
* **触摸选择复制**: 支持触摸框选终端内容并复制到剪贴板。
* **终端键盘**: 提供适配终端的软键盘扩展键，包括 `Esc`、`Tab`、方向键、`Ctrl`、`Alt`。
